...
<body oncontextmenu="return false;">
  ...
  <script>
        ...
        html += \`
          <div class="acc-card \${isDeadClass}" id="card-\${acc.SteamId}">
            ...
            <div class="acc-actions">
              ...
              \${(isWarrantyActive || acc.IsAlive === false) ? \`<button class="btn-acc-claim" onclick="claimWarrantyDirectly('\${safeToken}')">🛡️ ЗАМЕНА</button>\` : ''}
    ...
    async function checkAccountDirectly(encodedToken, steamId) {
      ...
      setStatus('loading', 'ПРОВЕРКА АККАУНТА...', 'Запрашиваем статус токена ' + steamId + ' в сети Steam...');
      const resJson = await bridge.CheckAccountLiveAsync(token);
      ...
      if (data.isAlive && data.secondsRemaining > 0) {
        ...
        setStatus('error', 'СЕССИЯ СБРОШЕНА ВЛАДЕЛЬЦЕМ', 'Вход невозможен. Гарантия активна — нажмите кнопку "ЗАМЕНА" для авто-выдачи нового аккаунта!');
...
console.log('Successfully updated index.html with live avatars and liveness check!');
