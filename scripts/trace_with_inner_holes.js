...
letterIntervals.forEach((item, idx) => {
  ...
  if (startX !== -1) {
    const outerPts = traceLoop(startX, startY, 1);
    ...
    // 2. Find and trace all inner holes for letters with holes (A, R, P, B)
    if (item.hasHole) {
      const holeVisited = new Set();
      for (let y = 15; y < height - 15; y += 3) {
        let insideSolid = false;
        for (let x = item.x1 + 3; x <= item.x2 - 3; x++) {
          if (grid(x, y) === 1 && grid(x + 1, y) === 0) {
            insideSolid = true;
          } else if (insideSolid && grid(x, y) === 0) {
            // Found transparent pixel inside solid letter boundary!
            const key = `${x},${y}`;
            if (!holeVisited.has(key)) {
              const holePts = traceLoop(x, y, 0);
              if (holePts.length > 4) {
                holePts.forEach(p => holeVisited.add(`${p.x},${p.y}`));
                d += ` M ${holePts[0].x} ${holePts[0].y}`;
                for (let j = 1; j < holePts.length; j++) {
                  d += ` L ${holePts[j].x} ${holePts[j].y}`;
                }
                d += ' Z';
              }
            }
...
console.log('Successfully traced letters with inner holes:', resultLetters.map(l => l.char).join(', '));
...
console.log('Updated src/data/sharpbuyLetters.js with full outer + inner hole paths!');
