...
<style>
  ...
  * { 
  ...
  /* Right: Expandable History Drawer (380px) */
  .history-drawer {
    width: 380px;
    min-width: 380px;
    background: #0d1015;
    ...
    <div class="history-drawer" id="historyDrawer">
      <div class="drawer-header">
        <div>
          ...
          <div class="drawer-subtitle">Вход в 1 клик, аватарки и статус гарантии</div>
        </div>
  ...
  <script>
    ...
    function renderAccountsList() {
      ...
      if (!savedAccountsCache || savedAccountsCache.length === 0) {
        container.innerHTML = '<div class="empty-history">У вас пока нет сохраненных аккаунтов.<br>Войдите в любой аккаунт, и он автоматически появится здесь с реальной аватаркой для входа в 1 клик!</div>';
        return;
      }
      ...
      savedAccountsCache.forEach((acc, index) => {
        ...
        if (acc.IsAlive === false) {
          warrantyBadge = '<span class="warranty-badge warranty-dead">🔴 СЕССИЯ СБРОШЕНА</span>';
        } else if (warrantySecondsLeft > 0) {
        ...
        html += \`
          <div class="acc-card \${isDeadClass}" id="card-\${acc.SteamId}">
            ...
            <div class="acc-actions">
              ...
              <button class="btn-acc-claim" id="btn-claim-\${acc.SteamId}" style="\${(isWarrantyActive || acc.IsAlive === false) ? '' : 'display:none;'}" onclick="claimWarrantyDirectly('\${safeToken}')">🛡️ ЗАМЕНА</button>
    ...
    async function checkAccountDirectly(encodedToken, steamId) {
      ...
      setStatus('loading', 'ПРОВЕРКА АККАУНТА...', 'Запрашиваем статус токена ' + steamId + ' в сети...');
      const resJson = await bridge.CheckAccountLiveAsync(token, steamId);
      const data = JSON.parse(resJson || '{}');

      if (chkBtn) chkBtn.innerText = 'ПРОВЕРИТЬ';
      ...
      if (data.isAlive && data.secondsRemaining > 0) {
        ...
        if (badgeContainer) badgeContainer.innerHTML = '<span class="warranty-badge warranty-active">🟢 АКТИВЕН</span>';
      } else {
        setStatus('error', 'СЕССИЯ СБРОШЕНА ВЛАДЕЛЬЦЕМ', 'Вход невозможен: ' + (data.reason || 'Сессия отозвана') + '. Нажмите "ЗАМЕНА" для авто-выдачи!');
        if (card) card.classList.add('dead-session');
        if (badgeContainer) badgeContainer.innerHTML = '<span class="warranty-badge warranty-dead">🔴 СЕССИЯ СБРОШЕНА</span>';
...
console.log('Successfully generated updated index.html with live status updates!');
